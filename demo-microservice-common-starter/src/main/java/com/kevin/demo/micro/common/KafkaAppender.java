 ...
 * 自动集成 Micrometer Tracing → Brave → MDC → Logback → Kafka
 * 无需手动处理 traceId/spanId，MDC 中已自动包含
 */
public class KafkaAppender extends AppenderBase<ILoggingEvent> {
    ...
    public void start() {
        ...
        try {
            ...
            props.put(ProducerConfig.ACKS_CONFIG, "1");  // 修改为1，确保可靠性
            ...
            encoder.start();
            super.start();
            addInfo("KafkaAppender started (OTel), sending logs to " + bootstrapServers + " topic: " + topic);
        } catch (Exception e) {
